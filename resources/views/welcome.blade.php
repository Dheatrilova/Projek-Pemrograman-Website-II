...
<body class="antialiased bg-white text-adv-dark font-sans">
    ...
    <section class="relative w-full min-h-[100svh] lg:min-h-[108svh] flex items-center overflow-hidden pt-20 md:pt-24 lg:pt-28">
        ...
        @endphp
        @if($isVideo)
            <video autoplay loop muted playsinline class="absolute inset-0 w-full h-full object-cover z-0" src="{{ $bgUrl }}"></video>
        @elseif($bgUrl)
            <img src="{{ $bgUrl }}" class="absolute inset-0 w-full h-full object-cover lg:[object-position:center_12%] z-0" alt="Banner Utama">
        @else
            <div class="absolute inset-0 w-full h-full bg-cover bg-center bg-no-repeat lg:[background-size:126%] lg:[background-position:center_12%] bg-[#F6F1E3] z-0" style="background-image: url('{{ asset('background.png') }}');"></div>
        @endif
        ...
        <div class="relative z-10 w-full px-6 py-8 md:px-16 lg:px-24 pb-24 md:pb-32 lg:pb-40">
            <div class="max-w-lg lg:max-w-md xl:max-w-lg bg-black/20 backdrop-blur-md border border-white/10 rounded-[1.75rem] md:rounded-[2rem] p-5 md:p-6 shadow-2xl shadow-black/20">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <a href="javascript:void(0)" class="bg-[#F0BB55] text-adv-brown px-5 py-2 rounded-full font-bold text-xs md:text-sm flex items-center gap-2 hover:brightness-110 transition duration-300 shadow-lg shadow-black/20">
                        Mulai Menjelajah
...
<form action="https://www.google.com/search" method="GET" target="_blank" class="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-[90%] lg:max-w-4xl z-30">
    <div class="bg-white rounded-full p-2 shadow-lg flex items-center justify-between border-2 border-white/80 w-full backdrop-blur-md">
        <div class="flex items-center flex-1 px-4">
            ...
            <input type="text" name="q" placeholder="Cari tempat, budaya, kuliner, atau hal menarik di Indonesia..." class="w-full bg-transparent outline-none text-sm md:text-base text-gray-700 placeholder-gray-400" required>
        </div>
        <div class="flex items-center gap-2 shrink-0">
            <button type="submit" class="bg-[#78481A] text-white p-2 md:p-3 rounded-full hover:bg-[#5C3612] transition shadow-md">
        ...
        <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
            ...
            <div class="lg:col-span-5 flex flex-col justify-center">
                ...
                <h2 class="text-5xl md:text-6xl lg:text-[5rem] font-serif font-bold text-[#B07A2A] leading-[1.1] drop-shadow-sm mb-6">
                    Jelajahi Pesona <br> Nusantara
                </h2>
                
                <hr class="border-t-[3px] border-[#B07A2A] w-20 mb-8">

                <p class="text-[#5C3A21] text-base md:text-lg leading-relaxed mb-10 max-w-md">
                   Temukan keajaiban alam, kekayaan budaya, dan sejarah bangsa dalam satu platform interaktif yang dikurasi khusus untuk petualang sejati
                </p>
...
<div>
            ...
            <div class="lg:col-span-7 grid grid-cols-2 md:grid-cols-3 grid-rows-[auto_auto] gap-4 md:gap-5 h-full">
                ...
                <div class="col-span-2 md:col-span-1 md:row-span-2 rounded-[2rem] overflow-hidden shadow-md">
                    <img src="{{ $pengaturan && $pengaturan->hero_gambar_1 ? asset($pengaturan->hero_gambar_1) : asset('potraitkiri.png') }}" alt="Portrait Destinasi" class="w-full h-full object-cover min-h-[300px] md:min-h-full hover:scale-105 transition duration-500">
                </div>
                
                <div class="col-span-2 md:col-span-2 rounded-[2rem] overflow-hidden shadow-md h-48 md:h-64 lg:h-[18rem]">
                    <img src="{{ $pengaturan && $pengaturan->hero_gambar_2 ? asset($pengaturan->hero_gambar_2) : asset('landscapeatas.png') }}" alt="Landscape Destinasi" class="w-full h-full object-cover hover:scale-105 transition duration-500">
                </div>
                
                <div class="col-span-1 rounded-[2rem] overflow-hidden shadow-md h-40 md:h-48 lg:h-56">
                    <img src="{{ $pengaturan && $pengaturan->hero_gambar_3 ? asset($pengaturan->hero_gambar_3) : asset('square.png') }}" alt="Square Destinasi 1" class="w-full h-full object-cover hover:scale-105 transition duration-500">
                </div>
                
                <div class="col-span-1 rounded-[2rem] overflow-hidden shadow-md h-40 md:h-48 lg:h-56">
                    <img src="{{ $pengaturan && $pengaturan->hero_gambar_4 ? asset($pengaturan->hero_gambar_4) : asset('square.png') }}" alt="Square Destinasi 2" class="w-full h-full object-cover hover:scale-105 transition duration-500">
                </div>
...
<section class="w-full bg-[#F6F1E3] py-16 pb-24">
        <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10">
            ...
            <div class="lg:col-span-5 relative rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-lg h-[400px] md:h-[480px] group">
                <img src="{{ asset('terumbu.jpg') }}" alt="Artikel Utama" class="w-full h-full object-cover group-hover:scale-105 transition duration-700">
                ...
                <div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#9B5C1B] via-[#9B5C1B]/90 to-transparent px-6 py-8 md:px-8 pt-24">
                    ...
                    <p class="text-white/90 text-xs md:text-sm leading-relaxed line-clamp-4">
                        Raja Ampat di Papua Barat memegang rekor sebagai wilayah dengan keanekaragaman hayati laut terkaya di planet bumi, rumah bagi 75% seluruh spesies karang yang ada di dunia.
                </div>
            </div>

            <div class="lg:col-span-7 flex flex-col justify-start lg:justify-between overflow-hidden">
                
                <h2 class="text-4xl md:text-5xl font-serif font-bold text-[#B07A2A] mb-6 mt-4 lg:mt-0 drop-shadow-sm">
                    Tahukah Kamu?
                </h2>
                
                <div class="flex flex-row gap-2 md:gap-4 flex-1 h-56 md:h-full w-full">
                    
                    @foreach($tahukahKamu as $item)
                    <div class="relative rounded-[1.5rem] overflow-hidden shadow-md transition-all duration-500 ease-in-out flex-1 hover:flex-[4] group cursor-pointer">
                        <img src="{{ $item->gambar ? asset($item->gambar) : asset('gambar.png') }}" alt="{{ $item->judul }}" class="absolute inset-0 w-full h-full object-cover">
                        
                        <div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#9B5C1B] via-[#9B5C1B]/90 to-transparent px-4 py-5 pt-16 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end">
                            <div class="w-40 md:w-56">
                                <h3 class="text-white text-lg md:text-xl font-bold mb-1 truncate">{{ $item->judul }}</h3>
                                <p class="text-white/90 text-[10px] md:text-xs line-clamp-3">
                                    {{ $item->deskripsi }}
                                </p>
                            </div>
                        </div>
...
<a href="{{ url('/wawasan-kuis') }}" class="inline-block border border-white/40 bg-gradient-to-r from-[#8B5920]/80 to-[#A8722B]/60 backdrop-blur-md text-white px-8 py-3.5 rounded-full font-bold text-base md:text-lg hover:from-[#8B5920] hover:to-[#8B5920] hover:shadow-[0_0_20px_rgba(139,89,32,0.5)] transition duration-300">
    Mulai Kuis
</a>
                </div>
...
<div class="fixed bottom-6 right-6 z-50">
    <button id="music-toggle" class="bg-[#B07A2A] text-white p-3 rounded-full shadow-lg hover:bg-[#5C3A21] transition-all duration-300 flex items-center justify-center">
        ...
        <svg id="icon-mute" class="w-6 h-6 hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" />
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 14l2-2m0 0l2-2m-2 2l2 2m-2-2l-2-2" />
        </svg>
    </button>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Menyesuaikan ikon awal dengan status pemutaran musik (dimatikan pada awalnya)
        if (bgMusic.paused) {
